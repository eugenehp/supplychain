...
/** @param {TopicSignature} current @param {TopicSignature} peer @param {{ key: string, label: string }[]} groupLegend */
function categoryBreakdownForPair(current, peer, groupLegend) {
  const keys = groupLegend.filter((g) => g.key !== 'product').map((g) => g.key);
  const rows = pairwiseSliceBreakdown(current, peer, sliceByGroup, keys);
  return rows.map(({ key, percent }) => {
    const legend = groupLegend.find((g) => g.key === key);
    return {
      key,
...
/**
 * Build similarity report from in-memory datasets (browser fallback or pipeline).
 * @param {string} currentTopicId
 * @param {object | null | undefined} currentData
 * @param {Map<string, object>} datasetsById
 * @param {object[]} topicMetas
 * @param {{ key: string, label: string }[]} groupLegend
 */
export function computeSimilarityReportFromDatasets(
  currentTopicId,
  currentData,
  datasetsById,
  topicMetas,
  ...
  const currentSig = currentData ? buildTopicSignature(currentData) : null;
  if (!currentSig) return { topics: [] };
  ...
  for (const meta of topicMetas) {
    ...
    const peerData = datasetsById.get(meta.id);
    if (!peerData) continue;
    const peerSig = buildTopicSignature(peerData);
    ...
    topics.push({
      ...
      categories: categoryBreakdownForPair(currentSig, peerSig, groupLegend),
    });
  }
