<script>
  ...
  import { loadSimilarityReport } from './topic-similarity.js';
  ...
  $effect(() => {
    ...
    loading = true;
    const gen = ++loadGen;
    loadSimilarityReport(id).then((result) => {
      if (gen === loadGen) {
        report = result ?? { topics: [] };
        loading = false;
      }
    });
  });
...
<section id="similarity" class="ui-section" aria-label="Supply chain similarity">
  <LazyInView minHeight="12rem">
    ...
    {#if loading}
      <div class="text-muted-foreground flex items-center gap-2 py-8 text-sm" aria-busy="true">
        <LoadingSpinner />
