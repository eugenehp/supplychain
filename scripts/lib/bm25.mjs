/** Okapi BM25 — shared server implementation */
...
export function rankByBm25(index, query, items, {
  getText = (x) => x.text ?? x.excerpt ?? '',
  getId = (x) => x.id,
  minScore = 0.1,
  limit = 20,
} = {}) {
  if (!query?.trim() || !index?.N) return [];

  const scores = index.scoreQuery(query);
  return items
    .map((item) => ({ item, score: scores.get(getId(item)) ?? 0 }))
